...
class GasParticle(Particle2D):
    ...
    def collide(self, particle2):
        ...
        dx, dy = self._x - particle2.getXPos(), self._y - particle2.getYPos()
        distanceOfCenters = self.centerDistance(self._x, self._y, particle2.getXPos(), particle2.getYPos())

        if distanceOfCenters < self._size + particle2.getSize():
            # calculate difference in components of velocity
            dvx, dvy = self._vx - particle2._vx, self._vy - particle2._vy
            ...
            offset = (self._size + particle2.getSize() - distanceOfCenters)/2
            dx2, dy2 = sin*offset, cos*offset

            self._x += dx2
            self._y += dy2
            particle2.setXPos(particle2.getXPos()-dx2)
            particle2.setYPos(particle2.getYPos()-dy2)
            ...
            self._vx += new_dvx
            self._vy += new_dvy
            particle2._vx -= new_dvx
            particle2._vy -= new_dvy
