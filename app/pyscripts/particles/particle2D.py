...
class Particle2D(Particle):
    ...
    def __init__(self, position, size, screen, screenW, screenH, screenPos, boundaryX, boundaryY, color):
        ...
        # set x and y components of velocity
        self.setVxVy()
    ...
    def validateSpeedXYInRange(self, value):
        # verifies user's input for and x and y velocities are within range
    ...
    def setVx(self, value):
        vx = self.validateSpeedXYInRange(value)
        self._vx = vx 

    def setVy(self, value):
        vy = self.validateSpeedXYInRange(value)
        self._vy = vy 
